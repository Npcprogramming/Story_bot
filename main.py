...
def update_referral(ref_id: int) -> bool:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user:
        ...
        # Если пользователь пришёл по реферальной ссылке, обновляем счётчик владельца
        if ref_id:
            if update_referral(ref_id):
                ...
                try:
                    ...
                    logger.info(f"Отправлена вторая часть истории владельцу ссылки {ref_id}")
                except Exception as e:
                    logger.error(f"Ошибка при отправке истории пользователю {ref_id}: {e}")
    else:
        logger.info(f"Пользователь {user_id} уже зарегистрирован.")

    # Если пользователь уже имеет доступ к истории (story_progress > 1), отправляем сообщение с кнопкой "Продолжить читать"
    ...
    # Если story_progress == 1, предлагаем приглашать друзей
    bot_username = context.bot.username
    referral_link = get_referral_link(bot_username, user_id)
    share_text = (
        "Привет! Я читаю рассказ \"Отель Спринг\" в боте, и мне нужно пригласить двух друзей, чтобы продолжить.\n\n"
    ...
    await update.message.reply_text(
        ...
        "Чтобы тоже хорошо провести время, пригласи двух своих знакомых по кнопке внизу.\n\n"
        "Как только они зайдут - здесь начнется сюжет.Тебе понравится ❤️",
        reply_markup=keyboard
    )
...
async def continue_story(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Получаем текущий прогресс истории
    cursor.execute("SELECT story_progress FROM users WHERE id = ?", (user_id,))
    result = cursor.fetchone()
    if not result:
    ...
    # Если для части истории указан фото, отправляем его с подписью
    ...
    # Если указан аудиофайл, пробуем его отправить
